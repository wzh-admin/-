const Calendar = {
  ...
  get firstDayOfWeek() { const d = new Date(this.currentYear, this.currentMonth - 1, 1); return d.getDay() === 0 ? 6 : d.getDay() - 1; },
  get monthKey() { return this.currentYear + '-' + String(this.currentMonth).padStart(2, '0'); },
  get todayStr() { const now = new Date(); return now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0'); },
  ...
  buildDayCell(day, dateStr, todayStr, isOther) {
    ...
    if (summary && summary.count > 0) {
      ...
      if (summary.income > 0) { var inc = summary.income; var incStr = (inc % 1 === 0) ? inc.toFixed(0) : inc.toFixed(1); amounts += '<span class="calendar-day__amt calendar-day__amt--income">+¥' + incStr + '</span>'; }
      if (summary.expense > 0) { var exp = summary.expense; var expStr = (exp % 1 === 0) ? exp.toFixed(0) : exp.toFixed(1); amounts += '<span class="calendar-day__amt calendar-day__amt--expense">-¥' + expStr + '</span>'; }
      amounts += '</div>';
    }
